...
import { Check, Search, ShieldAlert, Users, BarChart3, SlidersHorizontal } from "lucide-react";

import { base44 } from "@/api/base44Client";
import { createPageUrl } from "@/utils";
import { Button } from "@/components/ui/button";
import { getAdminFlags, saveAdminFlags } from "@/lib/admin-settings";
...
export default function Admin({ user }) {
  ...
  const [featureFlags, setFeatureFlags] = useState(() => getAdminFlags());
  ...
  const setFlag = (key, value) => {
    ...
    setFeatureFlags((previous) => ({ ...previous, [key]: value }));
  };

  const saveFeatureFlags = () => {
    saveAdminFlags(featureFlags);
    setFeatureFlags(getAdminFlags());
      ...
      <section className="glass-card p-5 space-y-4">
        ...
        <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            {settingsSaved && (
              <p className="text-sm text-emerald-600 flex items-center gap-1"><Check className="w-4 h-4" /> Saved</p>
            )}
            <Button onClick={saveFeatureFlags} className="bg-violet-600 hover:bg-violet-500 text-white">Save feature flags</Button>
          </div>
        </div>
      </section>
    </div>
