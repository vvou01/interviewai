  ...
  return (
    <div className="h-screen flex items-center justify-center p-6 bg-slate-50">
      <div className="text-center max-w-md">
        ...
        <h2 className="text-xl font-semibold text-slate-800 mb-1">Missing session ID</h2>
        <p className="text-sm text-slate-500 mb-5">Open this page with a valid session id in the URL.</p>
    ...
    <div className="mx-4 mt-4 rounded-xl border border-violet-200 bg-violet-50 p-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
      <div>
        ...
        <p className="text-xs text-violet-700 mt-1">
          Install and activate the extension to begin. Timer starts when the session becomes active.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <button
          onClick={onCopy}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded border border-violet-200 bg-white text-xs font-mono text-violet-800"
        >
          {sessionId}
          {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
        </button>

        <a href="#" className="text-xs text-violet-700 inline-flex items-center gap-1">
    ...
    <div className="fixed top-0 left-0 right-0 z-20 h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4">
      <div className="flex items-center gap-3 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-xs font-medium ${isActive ? "text-red-500" : "text-slate-400"}`}>
            {isActive ? "LIVE" : String(session.status || "setup").toUpperCase()}
          </span>
        </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={onEnd} className="bg-red-500 hover:bg-red-600">
                  End Session
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
...
export default function SessionActive({ user }) {
  ...
  const {
    ...
    queryFn: async () => {
      const sessions = await base44.entities.InterviewSessions.filter({ id: sessionId, created_by: user?.id });
  ...
  if (!sessionId) return <MissingSessionState />;

  if (!isLoadingSession && !session) return <NotFoundState />;
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-slate-50">
      <TopBar session={session} isActive={isActive} elapsed={elapsed} onEnd={handleEnd} />

      <div className="pt-14 flex-1 flex flex-col overflow-hidden">
        {isSetup && <SetupBanner sessionId={sessionId} copied={copied} onCopy={handleCopy} />}
        {isCompleted && <CompletedBanner sessionId={sessionId} />}
