  ...
  if (!questions || questions.length === 0) {
    return (
      <div className="glass-card p-6">
        <h2 className="font-semibold text-lg text-slate-800 mb-2">Question by Question</h2>
        <p className="text-sm text-slate-400">No question analysis available.</p>
      </div>
    );
  }
      ...
      <div className="divide-y divide-slate-100">
        {questions.map((qa, i) => {
          ...
          return (
            <div
              key={i}
              className={`px-5 py-4 ${i % 2 === 1 ? "bg-slate-50/60" : "bg-white"}`}
              onClick={() => isLong && setExpanded((e) => ({ ...e, [i]: !e[i] }))}
            >
